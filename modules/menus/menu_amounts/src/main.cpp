...
void onLoad() {
    l_menu = new AmountsMenu(*g_menuMgr->getPermanentData<Cursor>(), *g_menuMgr->getPersistentData<AmountsData>());
...
void onDelete() {
    auto data = g_menuMgr->getPersistentData<AmountsData>();
    delete data;
    g_menuMgr->setPersistentData<AmountsData>(nullptr);
}
