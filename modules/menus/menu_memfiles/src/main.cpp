...
void onLoad() {
    l_menu = new MemfilesMenu(*g_menuMgr->getPermanentData<Cursor>(), *g_menuMgr->getPersistentData<MemfilesData>());
...
void onDelete() {
    auto data = g_menuMgr->getPersistentData<MemfilesData>();
    delete data;
    g_menuMgr->setPersistentData<MemfilesData>(nullptr);
}
