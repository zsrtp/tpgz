...
void draw_ellipse(uint32_t color, Vec2 center, Vec2 dim) {
    ...
    for (uint8_t i = 0; i < 4; ++i) {
        for (size_t j = 0; j < n_points; ++j) {
            Draw::add_vertex(color, {center.x + dim.x / 2 * dx[j], center.y + dim.y / 2 * dy[j]}, {0.5f + dx[j], 0.5f + dy[j]});
            tmp = dx[j];
            dx[j] = -dy[j];
            dy[j] = tmp;
        }
...
void draw_button(Controller::Button button, char c, uint32_t color, Vec2 pos, Vec2 dim, float char_size) {
    ...
    Font::gz_renderChar(c, pos.x + (dim.x - width) / 2, pos.y + (dim.y + char_size / 2) / 2, is_button_down ? 0x00000060 : color,
                        false, char_size);
}

void draw_button_ellipse(Controller::Button button, char c, uint32_t color, Vec2 pos, Vec2 dim, float char_size) {
    bool is_button_down = Controller::button_is_down(button);
...
void draw_input_viewer(Vec2 pos, float scale, bool is_shadow) {
    ...
    draw_button_ellipse(Controller::C, 'c', is_shadow ? 0x00000060 : 0xFFFFFFFF,
                {pos.x + 42.5f * scale, pos.y + 15.f * scale}, {15.f * scale, 10.f * scale}, 8 * scale);
    draw_button(Controller::Z, 'Z', is_shadow ? 0x00000060 : 0xFFFFFFFF,
                {pos.x + 40.f * scale, pos.y + 30.f * scale}, {20.f * scale, 15.f * scale}, 8 * scale);
    draw_button_ellipse(Controller::A, 'A', is_shadow ? 0x00000060 : 0xBFBFBFFF,
                {pos.x + 70.f * scale, pos.y + 42.5f * scale}, {15.f * scale, 15.f * scale}, 8 * scale);
    draw_button(Controller::B, 'B', is_shadow ? 0x00000060 : 0xFFFFFFFF,
                {pos.x + 95.f * scale, pos.y + 40.f * scale}, {10.f * scale, 20.f * scale}, 8 * scale);
    draw_button_ellipse(Controller::MINUS, '-', is_shadow ? 0x00000060 : 0xFFFFFFFF,
                {pos.x + 115.f * scale, pos.y + 12.5f * scale}, {10.f * scale, 10.f * scale}, 8 * scale);
    draw_button_ellipse(Controller::HOME, ' ', is_shadow ? 0x00000060 : 0x00BFFFFF,
                {pos.x + 130.f * scale, pos.y + 12.5f * scale}, {10.f * scale, 10.f * scale}, 8 * scale);
    draw_button_ellipse(Controller::PLUS, '+', is_shadow ? 0x00000060 : 0xFFFFFFFF,
                {pos.x + 145.f * scale, pos.y + 12.5f * scale}, {10.f * scale, 10.f * scale}, 8 * scale);
    draw_button_ellipse(Controller::ONE, '1', is_shadow ? 0x00000060 : 0xFFFFFFFF,
                {pos.x + 127.5f * scale, pos.y + 30.f * scale}, {15.f * scale, 15.f * scale}, 8 * scale);
    draw_button_ellipse(Controller::TWO, '2', is_shadow ? 0x00000060 : 0xFFFFFFFF,
                {pos.x + 127.5f * scale, pos.y + 50.f * scale}, {15.f * scale, 15.f * scale}, 8 * scale);
    ...
    Draw::draw_rect_outline(is_shadow ? 0x00000060 : 0xFFFFFFFF, {pos.x, pos.y},
                            {35.f * scale, 7.f * scale}, OUTLINE_WIDTH);
    Draw::draw_rect(is_shadow ? 0x00000060 :
                                tp_mPad.nunchuck_shake > 0.2 ? 0x00FF00FF : 0xFFFFFFFF,
    ...
    Draw::draw_rect_outline(is_shadow ? 0x00000060 : 0xFFFFFFFF, {pos.x + 117.5f * scale, pos.y},
                            {35.f * scale, 7.f * scale}, OUTLINE_WIDTH);
    Draw::draw_rect(is_shadow ? 0x00000060 :
                                tp_mPad.wiimote_shake > 0.2 ? 0x00FF00FF : 0xFFFFFFFF,
