...
void test() {
    // Vec pos = dComIfGp_getPlayer()->mCurrent.mPosition;
    // Draw::draw3DLine(0x00FF00FF,pos);
...
KEEP_FUNC void execute() {
    if (g_moveLinkEnabled) {
        ...
        if (!init_once) {
            g_drawListener->addListener(test);
            init_once = true;
        }
        ...
        int16_t& link_horizontal_angle = dComIfGp_getPlayer()->mCollisionRot.mY;
        int16_t& link_verticle_angle = dComIfGp_getPlayer()->mCollisionRot.mX;
        ...
        g_drawListener->removeListener(test);
    }
}
